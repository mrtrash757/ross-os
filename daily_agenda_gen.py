...
def run():
    ...
    day_row_id = None  # Captured for use by step 4 (intent update)
    try:
        day_rows = coda_get(DAYS_TABLE)
        for r in day_rows:
            if today in str(r.get("values", {}).get("Date", "")):
                day_row_id = r["id"]
                break
        
        if not day_row_id:
            resp = coda_upsert(DAYS_TABLE, [{"cells": [
                {"column": "Date", "value": today},
            ]}])
            # Try to get the new row ID from the response
            added = resp.get("addedRowIds", [])
            if added:
                day_row_id = added[0]
            results["day_row_created"] = True
            print(f"   ✅ Created Day row for {today}")
        else:
            print(f"   ⏭️  Day row already exists for {today} (id: {day_row_id})")
    ...
    try:
        ...
        # Get existing habit logs (all rows — needed for today check + count/streak)
        all_logs = coda_get(HABIT_LOGS_TABLE)
        today_log_habits = set()
        # Build per-habit history for Count/Streak calculation
        from collections import defaultdict
        habit_history = defaultdict(list)  # habit_name → [{date, completed}]
        for log in all_logs:
            v = log.get("values", {})
            log_date = str(v.get("Date", ""))
            habit_nm = str(v.get("Habit", "")).strip()
            if today in log_date:
                today_log_habits.add(habit_nm)
            if habit_nm and log_date:
                habit_history[habit_nm].append({
                    "date": log_date[:10] if "T" in log_date else log_date[:10],
                    "completed": v.get("Completed?", False) is True,
                })

        print(f"   Existing logs for today: {len(today_log_habits)}")

        # Create missing habit logs (with Count/Streak seeded)
        new_logs = []
        for habit in active_habits:
            ...
            # Calculate Count (cumulative completed) and Streak from history
            history = sorted(habit_history.get(habit_name, []), key=lambda x: x["date"])
            cum_count = sum(1 for h in history if h["completed"])
            streak = 0
            for h in reversed(history):
                if h["completed"]:
                    ...
                    break
            # New row is Completed?=False, so streak resets to 0
            # Count stays at cumulative (will increment when user checks it off)
            ...
            new_logs.append({"cells": [
                ...
                {"column": "Count", "value": cum_count},
                {"column": "Streak", "value": 0},
            ]})
